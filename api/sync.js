...
// server.js already initialises admin; this module just grabs the instance.
// If it hasn't been initialised yet (test environment), we initialise here.
...
/**
 * POST /api/sync
 * Accepts a single item or a { batch: [] } envelope.
 */
...
/**
 * GET /api/sync/:userId
 * Returns the latest server-side progress snapshot.
 */
...
async function applyItem(db, batchOp, userId, item, results) {
  ...
  switch (item.type) {
    case 'progress': {
      // payload = { [moduleId]: { completedLessons, quizScores, percentComplete } }
      batchOp.set(
        progressRef,
        {
    ...
    case 'quiz_result': {
      // payload = { moduleId, quizId, score, total, percentage, passed, passedAt }
    ...
    case 'lesson_event': {
      // payload = { moduleId, lessonId, completedAt, totalLessons }
      const { moduleId, lessonId, completedAt, totalLessons } = item.payload;

      // Read current completedLessons to compute the new percent
